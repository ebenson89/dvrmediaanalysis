...
class MyWindow(QtWidgets.QMainWindow, Ui_MainWindow):
    def __init__(self):
        ...
        #self.clean_moviedict = cleanup_time(remove_list(get_data(media_file_name)))
        #self.main_movies_dataframe = dict_to_dataframe(clean_moviedict)
        #self.build_graph_button.clicked.connect(self.build_graph)

        #What graphs the user can choose from
        def graph_options(self):
            graph_lables_dict = get_data(graph_file_name)
            i = 0

            for graph in graph_lables_dict:
                self.graph_list.insertItem(i, graph)
